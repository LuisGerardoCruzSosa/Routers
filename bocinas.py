...
#***Get con Filtro Path
@router.get("/bocinas/{id}", status_code=status.HTTP_302_FOUND)
async def bocinas(id:int):
...
@router.get("/bocinas/", status_code=status.HTTP_302_FOUND)
async def bocinas(id:int):
    users=filter (lambda user: user.id == id, users_list)  #Función de orden superior
    try:
        return list(users)[0]
...
#***Post
@router.post("/bocinas/", response_model=User, status_code=status.HTTP_201_CREATED)
async def bocinas(user:User):
    ...
    #***Put
@router.put("/bocinas/", status_code=status.HTTP_202_ACCEPTED)
async def bocinas(user:User):
    ...
    #http://127.0.0.1:8000/bocinas/
        ...
        #***Delete
@router.delete("/bocinas/{id}", status_code=status.HTTP_200_OK)
async def bocinas(id:int):
