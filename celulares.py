...
users_list= [User(id=0,Modelo="S23 Ultra", Marca="Samsung", Color="Azul"),
             User(id=1,Modelo="14 Pro Max", Marca="iPhone", Color="Blanco"),
             User(id=2,Modelo="Redmi Note 8", Marca="Xiaomi", Color="Negro")]
...
async def celulares():
    return (users_list)
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/celulares/


#***Get con Filtro Path
@router.get("/celulares/{id}", status_code=status.HTTP_302_FOUND)
async def celulares(id:int):
    ...
    try:
     ...
     # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/celulares/1


#***Get con Filtro Query
@router.get("/celulares/", status_code=status.HTTP_302_FOUND)
async def celulares(id:int):
    users=filter (lambda user: user.id == id, users_list)  #Función de orden superior
    try:
        return list(users)[0]
 ...
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/celulares/?id=1
 
 
#***Post
@router.post("/celulares/", response_model=User, status_code=status.HTTP_201_CREATED)
async def celulares(user:User):
    ...
    #http://127.0.0.1:8000/celulares/
   
   
    #***Put
@router.put("/celulares/", status_code=status.HTTP_202_ACCEPTED)
async def celulares(user:User):
    ...
    #http://127.0.0.1:8000/celulares/
        ...
        #***Delete
@router.delete("/celulares/{id}", status_code=status.HTTP_200_OK)
async def celulares(id:int):
    ...
    #http://127.0.0.1:8000/celulares/1