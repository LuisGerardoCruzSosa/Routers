...
async def ciudades():
    return (users_list)
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/ciudades/


#***Get con Filtro Path
@router.get("/ciudades/{id}", status_code=status.HTTP_302_FOUND)
async def ciudades(id:int):
    ...
    try:
     ...
     # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/ciudades/1


#***Get con ciudades Query
@router.get("/ciudades/", status_code=status.HTTP_302_FOUND)
async def ciudades(id:int):
    users=filter (lambda user: user.id == id, users_list)  #Función de orden superior
    try:
        return list(users)[0]
 ...
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/ciudades/?id=1
 
 
#***Post
@router.post("/ciudades/", response_model=User, status_code=status.HTTP_201_CREATED)
async def ciudades(user:User):
    ...
    #***Put
@router.put("/ciudades/", status_code=status.HTTP_202_ACCEPTED)
async def ciudades(user:User):
    ...
    #http://127.0.0.1:8000/ciudades/
        ...
        #***Delete
@router.delete("/ciudades/{id}", status_code=status.HTTP_200_OK)
async def ciudades(id:int):
    ...
    #http://127.0.0.1:8000/ciudades/1