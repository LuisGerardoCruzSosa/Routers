...
#-uvicorn computadoras:app --reload-
...
async def computadoras():
    return (users_list)
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/computadoras/


#***Get con Filtro Path
@router.get("/computadoras/{id}", status_code=status.HTTP_302_FOUND)
async def computadoras(id:int):
    ...
    try:
     ...
     # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/computadoras/1


#***Get con Filtro Query
@router.get("/computadoras/", status_code=status.HTTP_302_FOUND)
async def computadoras(id:int):
    users=filter (lambda user: user.id == id, users_list)  #Función de orden superior
    try:
        return list(users)[0]
 ...
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/computadoras/?id=1
 
 
#***Post
@router.post("/computadoras/", response_model=User, status_code=status.HTTP_201_CREATED)
async def computadoras(user:User):
    ...
    #***Put
@router.put("/computadoras/", status_code=status.HTTP_202_ACCEPTED)
async def computadoras(user:User):
    ...
    #http://127.0.0.1:8000/computadoras/
        ...
        #***Delete
@router.delete("/computadoras/{id}", status_code=status.HTTP_200_OK)
async def computadoras(id:int):
    ...
    #http://127.0.0.1:8000/computadoras/1