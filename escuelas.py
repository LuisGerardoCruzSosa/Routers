...
async def escuelas():
    return (users_list)
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/escuelas/


#***Get con Filtro Path
@router.get("/escuelas/{id}", status_code=status.HTTP_302_FOUND)
async def escuelas(id:int):
...
@router.get("/escuelas/", status_code=status.HTTP_302_FOUND)
async def escuelas(id:int):
    users=filter (lambda user: user.id == id, users_list)  #Función de orden superior
    try:
        return list(users)[0]
 ...
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/escuelas/?id=1
 
 
#***Post
@router.post("/escuelas/", response_model=User, status_code=status.HTTP_201_CREATED)
async def escuelas(user:User):
    ...
    #http://127.0.0.1:8000/escuelas/
   
   
    #***Put
@router.put("/escuelas/", status_code=status.HTTP_202_ACCEPTED)
async def escuelas(user:User):
    ...
    #http://127.0.0.1:8000/escuelas/
        ...
        #***Delete
@router.delete("/escuelas/{id}", status_code=status.HTTP_200_OK)
async def escuelas(id:int):
    ...
    #http://127.0.0.1:8000/escuelas/1